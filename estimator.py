...
class Gen6DMulEstimator(Gen6DEstimator):
    def __init__(self,cfg):
        super().__init__(cfg)
    ...
    def predict(self, que_img, que_K, pose_init=None):
        ...
        with torch.no_grad():
            detection_outputs = self.detector.detect_que_imgs(que_img)
        inter_results=[]
        poses = []
        for detection_output in  detection_outputs:
            position = detection_output['positions'][0]
            scale_r2q = detection_output['scales'][0]
            inter_result = {}
            # crop the image according to the detected scale and the detected position
            que_img_, _ = transformation_crop(que_img, position, 1/scale_r2q, 0, self.cfg['ref_resolution'])  # h,w,3
            inter_result['det_position'] = position
            inter_result['det_scale_r2q'] = scale_r2q
            inter_result['det_que_img'] = que_img_
            ...
            inter_result['sel_angle_r2q'] = angle_r2q
            inter_result['sel_scores'] = scores
            inter_result['sel_ref_idx'] = ref_idx
            ...
            inter_results.append(inter_result)
        print('Poses',len(poses))
        print('inter_results',len(inter_results))
        return poses, inter_results
    

name2estimator={
    'gen6d': Gen6DEstimator,
    'gen6d_mult': Gen6DMulEstimator,
}