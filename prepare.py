...
def video2image(input_video, output_dir, interval=30, image_size = 640, transpose=False):
    ...
    while success:
        if count % interval==0:
            ...
            imsave(f"{output_dir}/frame{img_count}.jpg", image)  # 使用连续的编号
            img_paths.append(f"{output_dir}/frame{img_count}.jpg")
            img_count += 1  # 每保存一张图片，编号加1
        success, image = vidcap.read()
        count += 1
    return img_paths
